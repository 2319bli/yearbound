...
func run() -> void:
	...
	await physics_frame
	app.start_lab()
	for station in range(11,17):
		var id="lab-"+str(station+1)
		app.lab_station(station)
		var w=app.world
		var flooded=w.spec.zones.any(func(z): return z.type=="current" and z.force[1]<-1000)
		var furthest=0.0
		var old_deaths=w.deaths
		var initial_deaths=w.deaths
		var jumped_at=-1
		var launch_count=0
		Input.action_press("right")
		for i in 18000:
			await physics_frame
			if w.complete or w.player.position.x>w.spec.lab_stations[station].end-80: break
			var p=w.player
			...
			if w.deaths>old_deaths:
				...
				old_deaths=w.deaths
				if old_deaths-initial_deaths>18: break
			...
			if p.is_on_floor() and w.respawn_delay<=0:
				...
				for h in w.spec.hazards:
					if h.type=="bramble" and h.x-p.position.x>100 and h.x-p.position.x<(140 if station==12 else 400) and h.y<p.position.y+80 and h.y+h.h>p.position.y-60:
		...
		print("ROUTE ",id," complete=",w.complete or w.player.position.x>w.spec.lab_stations[station].end-80," farthest=",furthest," deaths=",w.deaths," dash_releases=",launch_count)
		if not w.complete and w.player.position.x<=w.spec.lab_stations[station].end-80: failures += 1
	root.remove_child(app);app.queue_free();await process_frame;await process_frame
	print("EXTENDED LAB ROUTES TEST COMPLETE: ",failures," failures")
