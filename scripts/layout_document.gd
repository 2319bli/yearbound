...
func inside(at: Vector2i) -> bool: return at.x>=0 and at.x<columns() and at.y>=0 and at.y<ROWS
...
func paint(at: Vector2i, tool: String) -> void:
	...
	if tool=="erase":
		...
		var h={"type":"bramble","x":pos.x,"y":pos.y+24,"w":48,"h":24}
...
func paint_rectangle(a: Vector2i,b: Vector2i,tool: String) -> void:
	for y in range(maxi(0,mini(a.y,b.y)),mini(ROWS-1,maxi(a.y,b.y))+1):
...
static func errors(s: Variant, check_playable: bool=false) -> PackedStringArray:
	...
	if not number(s.get("length")) or s.length<1296 or s.length>MAX_COLUMNS*TILE or fmod(s.length,TILE)!=0: return PackedStringArray(["Length must be 27–1024 blocks."])
	...
	for p in s.platforms:
		...
		if p.x<0 or p.y<0 or p.w<=0 or p.h<=0 or p.x+p.w>s.length or p.y+p.h>720: return PackedStringArray(["A terrain block is outside the stage bounds."])
	...
	for pos in [s.spawn,s.goal]+s.checkpoints:
		if pos[0]<12 or pos[0]>s.length-12 or pos[1]<48 or pos[1]>720: out.append("A start, exit or checkpoint is outside the playable area.");continue
